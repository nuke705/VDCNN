...
train = pd.read_csv(dp1, header=None, usecols=[0,2])
...
test = pd.read_csv(dp2, header=None, usecols=[0,2])
...
for epoch in range(20):  # loop over the dataset multiple times
    ...
    for i,data in enumerate(trainloader, 0):
        ...
        if i % 1000 == 999 and (i != 1):    # print every 
            print('[%d, %5d] loss: %.3f time: %.4f' %
                 (epoch + 1, i + 1, running_loss / 1000, time.time()-time0))
            running_loss = 0.0
    ...
    print('Epoch',epoch + ,'Accuracy : %f %%' % (
        100 * correct / total))
    torch.save(net,'../model/dp_1_9_temp.model')
# model names: dataset _ downsampleing option _ layers
torch.save(net,'../model/dp_1_9_final.model')
