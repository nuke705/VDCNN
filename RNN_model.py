...
class StatefulLSTM(nn.Module):
    ...
    def forward(self,x):
        ...
        if self.h is None:
            ...
            #print(state_size)
            #print(Variable(torch.zeros(state_size)))
...
class LockedDropout(nn.Module):
    ...
    def forward(self, x, dropout=0.7, train=True):
        if train==False:
            return x
        if(self.m is None):
...
class LSTM_model(nn.Module):
    def __init__(self,insize,no_of_hidden,n_classes):
        super(LSTM_model, self).__init__()

        self.embedding = nn.Embedding(insize, no_of_hidden)
        
        self.lstm1 = StatefulLSTM(no_of_hidden,no_of_hidden)
        self.bn_lstm1= nn.BatchNorm1d(no_of_hidden)
        ...
        self.fc_output = nn.Linear(no_of_hidden, n_classes)
    ...
    def forward(self, x, train=True):
        ...
        for i in range(no_of_timesteps):
            ...
            h = self.bn_lstm1(h)
            h = self.dropout1(h,dropout=0.7,train=train)
