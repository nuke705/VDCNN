...
#yahoo news
#y1h = './datasets/yahoo_answers_csv/train.csv'
...
net = torch.load('dp_1_17_epoch6.model')
net.to(device)
...
for epoch in range(20):  # loop over the dataset multiple times
    ...
    for i,data in enumerate(trainloader, 0):
        ...
        if i % 1000 == 999 and (i != 1):    # print every 
            print('[%d, %5d] loss: %.3f time: %.4f' %
                 (epoch + 6 + 1, i + 1, running_loss / 1000, time.time()-time0))
            running_loss = 0.0
    ...
    print('Epoch',epoch + 7,'Accuracy : %f %%' % (
        100 * correct / total))
    torch.save(net,'../model/dp_1_17_temp.model')
# model names: dataset _ downsampleing option _ layers
torch.save(net,'../model/dp_1_17_final.model')
