...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 30 00:12:27 2018
...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 30 00:08:30 2018
...
#dbpedia
dp1 = './datasets/dbpedia_csv/train.csv'
dp2 = './datasets/dbpedia_csv/test.csv'
train = pd.read_csv(dp1, header=None, usecols = [0,1,2],parse_dates = [[1,2]])
...
test = pd.read_csv(dp2, header=None, usecols = [0,1,2],parse_dates = [[1,2]])
...
for epoch in range(20):  # loop over the dataset multiple times
    ...
    for i,data in enumerate(trainloader, 0):
        ...
        if i % 200 == 199 and (i != 1):    # print every 
            print('[%d, %5d] loss: %.3f time: %.4f' %
                 (epoch + 1, i + 1, running_loss / 200, time.time()-time0))
            running_loss = 0.0
    ...
    print('Epoch',epoch,'Accuracy : %f %%' % (
        100 * correct / total))
    torch.save(net,'../model/dbp_1_29_epoch{}.model'.format(epoch))
    
# model names: dataset _ downsampleing option _ layers
torch.save(net,'../model/dbp_1_29_final.model')
