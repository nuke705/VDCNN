...
downsample_option = '1'  #if using shortcut, make this = 1 as paper said
...
scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[3,6 ,8,9,10,11,12,13,14,15], gamma=0.4)                                                            
...
for epoch in range(20):  # loop over the dataset multiple times
    ...
    print('Epoch',epoch,'Accuracy : %f %%' % (
        100 * correct / total))
    #st = 'hanning_' + str(num) + '.pdf'
    torch.save(net,'ypf_1_29_s_'+str(epoch)+'.model')
    #torch.save(net,'ypf_1_29_s.model')
# model names: dataset _ downsampleing option _ layers
torch.save(net,'ypf_1_29_s_last.model')
time2 = time.time()
print(time2-time1)
