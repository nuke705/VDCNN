...
train = pd.read_csv(ag1, header=None, usecols=[0,1,2],parse_dates=[[1,2]])
...
test = pd.read_csv(ag2, header=None, usecols=[0,1,2],parse_dates=[[1,2]])
...
for epoch in range(20):  # loop over the dataset multiple times
    ...
    print('Epoch',epoch,'Accuracy : %f %%' % (
        100 * correct / total))
    torch.save(net,'./agmodel/ag_1_17_epoch{}.model'.format(epoch))
# model names: dataset _ downsampleing option _ layers
torch.save(net,'./agmodel/ag_1_17_final.model')
